...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	if(!$connection) {
        ...
        $sql="SELECT * FROM $database.Teams WHERE team='$team_name'";
        ...
        if($count == 0){
		    $sql="Insert INTO $database.Teams(team_name,descriptions) VALUES ($team_name,$descriptions)";
            $retval = mysqli_query( $connection, $sql );
            if(! $retval ) {
                echo "Error access in table Teams".mysqli_error($connection);
            }else{
...
<div class="modal fade" id="AddTeam" tabindex="-1" role="dialog" aria-labelledby="AddTeamLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
	  ...
	  <form method="post" class="TeamForm" novalidate>
